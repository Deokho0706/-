...
# ─────────────────────────────
#
# SCENARIO_COUNT determines how many Monte‑Carlo scenarios are run when
# simulating investment outcomes.  Increasing this value produces smoother
# distributions but at the cost of additional computation.  The previous
# version of this application used 5,000 scenarios, but based on user
# feedback we raise this to 20,000 to improve statistical robustness.
...
def build_targets_auto(
    ...
    """
    Automatically derive annual return targets for each ticker based on
    historical returns.  A haircut is applied to risky assets to reflect
    more conservative expectations.  Any cash‑like asset is assigned a
    fixed target equal to `cash_target_pp`.  Historically this function
    also enforced upper and lower bounds on returns (cap/floor), but
    those constraints have been removed to better utilize the full
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        with st.expander("🔧 고급 설정", expanded=False):
            ...
            # 리스크 상한/하한 슬라이더는 더 이상 사용하지 않으며 UI에서 제거되었습니다.
